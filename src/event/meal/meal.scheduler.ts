...
export async function setSchedule() {
  const scheduleBreakfast = schedule.scheduleJob("0 0 1 * * *", setNewMeal);
  const scheduleLunch = schedule.scheduleJob("0 0 5 * * *", setNewMeal);
  const sheduleDinner = schedule.scheduleJob("0 0 10 * * *", setNewMeal);
}
